...
def list_clients(
    ...
    clients = client_crud.get_clients(db, skip, limit, search)
    if status:
...
@router.get("/search")
def search_clients(q: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    """Search clients by name, email, or company"""
    return client_crud.get_clients(db, search=q)
...
def get_client_metrics(client_id: int, db: Session = Depends(get_db)):
...
def create_client_contact(client_id: int, contact: ClientContactCreate, db: Session = Depends(get_db)):
...
def list_client_contacts(client_id: int, db: Session = Depends(get_db)):
    ...
    return client_crud.get_client_contacts(db, client_id)
...
def create_client_contract(client_id: int, contract: ClientContractCreate, db: Session = Depends(get_db)):
...
def list_client_contracts(client_id: int, db: Session = Depends(get_db)):
    """Get all contracts for a client"""
    return client_crud.get_client_contracts(db, client_id)
...
def list_invoices(
    ...
    invoices = client_crud.get_invoices(db, client_id, skip, limit)
    if status:
...
def update_invoice(invoice_id: int, invoice: dict, db: Session = Depends(get_db)):
    """Update an invoice"""
...
def list_communications(
    ...
    return client_crud.get_communications(db, client_id, project_id)
...
def export_client(client_id: int, db: Session = Depends(get_db)):
    """Export client data"""
    client = client_crud.get_client_detail(db, client_id)
    if not client:
        raise HTTPException(status_code=404, detail="Client not found")
